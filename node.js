...
function Node(vec) {
	...
	while (note>Flute.prototype.range.high){
		note-=12;
	}
	while (note<ContraBassoon.prototype.range.low){
		note+=12;
	}
	...
	possibilities.forEach(function(p){
		if(note>=p.range.low && note<=p.range.high){
			withinRange.push(p)
		}
	})
	console.log(['within range: note is' + note, withinRange]);


	this.i = withinRange[Math.floor(Math.random()*withinRange.length)];




	// if (note < Bassoon.prototype.range.low) {
	// 	this.i = new ContraBassoon(ac);
	// } else if (note < CorAnglais.prototype.range.low) {
	// 	this.i = new Bassoon(ac);
	// } else if (note<Flute.prototype.range.low) {
